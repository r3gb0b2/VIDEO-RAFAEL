...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  ...
  [GenerationMode.SOCIAL_PROMO]: <SocialModeIcon className="w-5 h-5" />,
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  // Social Promo specific states
  const [promoText, setPromoText] = useState(initialValues?.promoText ?? 'INGRESSO PROMOCIONAL R$50');
  const [promoPrice, setPromoPrice] = useState(initialValues?.promoPrice ?? 'EQUIPE CERTA');
  const [promoPercent, setPromoPercent] = useState(initialValues?.promoPercent ?? 94);
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setPromoText(initialValues.promoText ?? 'INGRESSO PROMOCIONAL R$50');
      setPromoPrice(initialValues.promoPrice ?? 'EQUIPE CERTA');
      setPromoPercent(initialValues.promoPercent ?? 94);
    }
  }, [initialValues]);
  ...
  useEffect(() => {
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      ...
      setResolution(Resolution.P720);
    } else if (generationMode === GenerationMode.SOCIAL_PROMO) {
  ...
  const handleSelectMode = (mode: GenerationMode) => {
    ...
    setIsModeSelectorOpen(false);
    // Only reset if switching to radically different modes
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.REFERENCES_TO_VIDEO]: 'Describe a video using reference images...',
    [GenerationMode.SOCIAL_PROMO]: 'Additional details for the promo animation...',
  ...
  const selectableModes = [
    ...
    GenerationMode.REFERENCES_TO_VIDEO,
    GenerationMode.SOCIAL_PROMO,
  ];

  const renderMediaUploads = () => {
    if (generationMode === GenerationMode.FRAMES_TO_VIDEO || generationMode === GenerationMode.SOCIAL_PROMO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-col items-center justify-center gap-4">
          <div className="flex items-center justify-center gap-4">
            <ImageUpload
              label={generationMode === GenerationMode.SOCIAL_PROMO ? "Base Poster" : "Start Frame"}
              image={startFrame}
            ...
            />
            {generationMode === GenerationMode.FRAMES_TO_VIDEO && !isLooping && (
              <ImageUpload
                label="End Frame"
                image={endFrame}
          ...
          {generationMode === GenerationMode.SOCIAL_PROMO && startFrame && (
            <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-3 mt-2 border-t border-gray-700 pt-4">
                <div className="flex flex-col">
                    <label className="text-xs text-gray-400 mb-1">Blinking Label</label>
                    <input 
                        className="bg-[#1f1f1f] border border-gray-600 rounded-lg p-2 text-sm text-gray-200"
                        value={promoText}
                        onChange={e => setPromoText(e.target.value)}
                        placeholder="INGRESSO PROMOCIONAL R$50"
                    />
                </div>
                <div className="flex flex-col">
                    <label className="text-xs text-gray-400 mb-1">Price/Subtitle</label>
                    <input 
                        className="bg-[#1f1f1f] border border-gray-600 rounded-lg p-2 text-sm text-gray-200"
                        value={promoPrice}
                        onChange={e => setPromoPrice(e.target.value)}
                        placeholder="EQUIPE CERTA"
                    />
                </div>
                <div className="flex flex-col">
                    <label className="text-xs text-gray-400 mb-1">Loading Progress (%)</label>
                    <div className="flex gap-2">
                        ...
                        />
                        <span className="text-sm font-mono text-indigo-400 w-8">{promoPercent}%</span>
                    </div>
                </div>
            </div>
          )}

          {generationMode === GenerationMode.FRAMES_TO_VIDEO && startFrame && !endFrame && (
    ...
    if (generationMode === GenerationMode.EXTEND_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex items-center justify-center gap-4">
          <VideoUpload
            label={<>Input Video<br />(720p required)</>}
            video={inputVideo}
  ...
  const isPromoMode = generationMode === GenerationMode.SOCIAL_PROMO;
  ...
  switch (generationMode) {
    ...
    case GenerationMode.FRAMES_TO_VIDEO:
      ...
      break;
    case GenerationMode.SOCIAL_PROMO:
        isSubmitDisabled = !startFrame;
        if (isSubmitDisabled) tooltipText = 'A base poster image is required.';
        break;
        ...
        <div className="absolute bottom-full left-0 right-0 mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <CustomSelect
              ...
              disabled={isRefMode || isExtendMode || isPromoMode}>
